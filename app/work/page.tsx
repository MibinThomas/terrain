    ...
    <main className="relative min-h-screen bg-terrain-deepBlack text-terrain-softWhite">
      ...
      <section className="pt-40 pb-16 sm:pb-20 border-b border-white/5 relative overflow-hidden">
        <div className="container mx-auto px-4 sm:px-6 md:px-12 relative z-10">
          ...
          <p className="text-terrain-midGrey text-base sm:text-lg max-w-2xl leading-relaxed">
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
            {PROJECTS.map((p) => (
              ...
              >
                <div className="relative aspect-[16/9] sm:aspect-[2.2/1] overflow-hidden bg-terrain-nearBlack">
                  <img
                    src={p.image}
                    alt={p.title}
                ...
                <div className="p-6 sm:p-8 flex flex-col justify-between flex-grow">
                  <div>
                    <span className="text-[11px] sm:text-xs uppercase tracking-widest text-white font-semibold block mb-2">
                      {p.category}
                    </span>
                    <h3 className="font-heading font-bold text-xl sm:text-2xl text-terrain-softWhite mb-3 group-hover:text-white transition-colors">
                      {p.title}
                    </h3>
                  ...
                  <div className="pt-4 border-t border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                      ...
                      <span className="text-xs sm:text-sm font-semibold text-terrain-softWhite break-all">{p.result}</span>
                    </div>
                    <a
                      href={p.link}
                      target="_blank"
