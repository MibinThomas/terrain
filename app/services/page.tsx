...
export default function ServicesPage() {
  return (
    <main className="relative min-h-screen bg-terrain-deepBlack text-terrain-softWhite overflow-hidden">
      ...
      {/* Hero Section */}
      <section className="pt-40 sm:pt-44 pb-16 sm:pb-24 border-b border-white/5 relative">
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-12 relative z-10">
          ...
          <h1 className="font-heading font-extrabold text-3xl sm:text-6xl lg:text-7xl max-w-4xl leading-tight text-terrain-softWhite mb-6 tracking-tight">
          ...
          <p className="text-terrain-midGrey text-base sm:text-xl max-w-2xl leading-relaxed mb-10">
          ...
          {/* Quick Metrics Bar */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 max-w-3xl pt-6 border-t border-white/10">
            {[
              ...
              <div key={stat.label} className="bg-white/[0.02] border border-white/5 rounded-2xl p-3.5 sm:p-4 text-center">
                <span className="font-heading font-extrabold text-lg sm:text-2xl text-white block">
                  {stat.val}
                </span>
      ...
      <section className="py-20 sm:py-28 relative">
        <div className="container mx-auto px-4 sm:px-6 md:px-12 space-y-8 sm:space-y-12">
          {SERVICES.map((s) => {
            const Icon = s.icon;
            return (
              <div
                key={s.id}
                id={s.id}
                className="group relative bg-white/[0.02] backdrop-blur-xl border border-white/5 hover:border-white/30 rounded-3xl p-6 sm:p-10 md:p-12 transition-all duration-500 hover:shadow-[0_0_50px_rgba(255,255,255,0.08)] overflow-hidden"
              >
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start relative z-10">
                  {/* Service Core Info */}
                  <div className="lg:col-span-7 space-y-6">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center shadow-inner shrink-0">
                        <Icon size={24} className="text-white sm:w-7 sm:h-7" />
                      </div>
                      <span className={`text-[11px] sm:text-xs uppercase tracking-widest font-bold px-3 py-1 rounded-full border ${s.badgeColor}`}>
                        {s.id.replace("-", " ")}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-terrain-lightGrey text-xs sm:text-sm font-semibold mb-3">
                        {s.short}
                      </p>
                  ...
                  <div className="lg:col-span-5 bg-white/[0.03] border border-white/10 rounded-2xl p-6 sm:p-8 flex flex-col justify-between h-full">
                    <div>
                      <h3 className="text-xs uppercase tracking-widest text-terrain-midGrey font-bold mb-4 sm:mb-5 flex items-center gap-2">
                        <Layers size={14} className="text-white" /> Core Deliverables
                      </h3>
                      <ul className="space-y-3 text-xs sm:text-sm text-terrain-softWhite">
                    ...
                    <Link
                      href="/contact"
                      className="mt-6 sm:mt-8 w-full bg-white hover:bg-neutral-200 text-black text-xs font-bold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.2)]"
                    >
      ...
      <section className="py-20 sm:py-24 bg-terrain-nearBlack border-t border-white/5 relative">
        <div className="container mx-auto px-4 sm:px-6 md:px-12">
          <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16">
            ...
            <h2 className="font-heading text-2xl sm:text-5xl font-extrabold text-terrain-softWhite mb-3 sm:mb-4">
              Our 4-Step Engineering Protocol
            </h2>
            <p className="text-terrain-midGrey text-xs sm:text-base">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {PROTOCOL_STEPS.map((step) => (
              <div
                key={step.num}
                className="bg-white/[0.02] border border-white/5 hover:border-white/30 rounded-2xl p-5 sm:p-6 transition-all duration-300"
              >
                <span className="font-heading font-extrabold text-2xl sm:text-3xl text-white/30 block mb-3">
                  {step.num}
                </span>
                <h3 className="font-heading font-bold text-base sm:text-lg text-terrain-softWhite mb-2">
                  {step.title}
                </h3>
      ...
      <section className="py-16 sm:py-20 relative">
        <div className="container mx-auto px-4 sm:px-6 md:px-12">
          <div className="relative rounded-3xl bg-neutral-900/80 border border-white/20 p-8 sm:p-16 text-center overflow-hidden shadow-[0_0_80px_rgba(255,255,255,0.1)]">
            <div className="relative z-10 max-w-2xl mx-auto space-y-5 sm:space-y-6">
              ...
              <h2 className="font-heading font-extrabold text-2xl sm:text-5xl text-terrain-softWhite leading-tight">
                Ready to Build Your Next Product?
              </h2>
              <p className="text-terrain-midGrey text-xs sm:text-base leading-relaxed">
              ...
              <div className="pt-2 sm:pt-4 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
                <Link
                  href="/contact"
                  className="w-full sm:w-auto bg-white hover:bg-neutral-200 text-black font-bold px-8 py-4 rounded-xl text-sm transition-all duration-300 shadow-[0_0_30px_rgba(255,255,255,0.3)] flex items-center justify-center gap-2"
                >
                  Schedule a Consultation <ArrowRight size={16} />
                </Link>
                <Link
                  href="/work"
                  className="w-full sm:w-auto bg-white/5 hover:bg-white/10 text-terrain-softWhite border border-white/10 font-bold px-8 py-4 rounded-xl text-sm transition-all duration-300 text-center"
                >
